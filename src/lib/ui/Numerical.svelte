<script>
  ...
  const { start, stop, started } = useFrame(
    () => {
      value = Math.round((value + step) * (1 / step)) / (1 / step); // Round to step precision
      step = value === max || value === min ? -step : step; // Reverse velocity when value reaches min or max
    },
    {
      autostart: false,
    },
...
<div class="my-2">
  ...
  <span class="py-2 flex gap-2 justify-between items-center">
    <input class="text-right" type="text" bind:value={min} size={`${min}`.length} />
